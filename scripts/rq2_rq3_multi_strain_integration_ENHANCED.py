...
python rq2_rq3_multi_strain_integration_ENHANCED.py \\
    ...
    # Explicit per-strain file paths passed by the master orchestrator.
    # Format: 'Strain1:/path/to/file.csv,Strain2:/path/to/file.csv'
    # When provided these paths take priority over auto-constructed ones,
    # making the script independent of any particular folder layout on disk.
    parser.add_argument('--strain_files', type=str, default=None,
                       help=(
                           'Explicit per-strain file paths as a comma-separated '
                           'Strain:path list.  When supplied, overrides the '
                           'auto-constructed path for every strain in the list.'
                       ))

    # GEO dataset identifier used to build per-strain paths automatically.
    # Only relevant when --strain_files is NOT provided.
    parser.add_argument('--rq2_dataset_id', type=str, default='GSE182668',
                       help=(
                           'GEO accession used when auto-building per-strain paths: '
                           '{base_dir}/results_{strain}_{rq2_dataset_id}/flux_analysis/'
                           'reaction_flux_comparison_extended.csv  (default: GSE182668)'
                       ))
    ...
    # ------------------------------------------------------------------
    # Build strain -> file-path mapping.
    #
    # Priority order:
    #   1. --strain_files  (explicit paths from the master orchestrator)
    #   2. Auto-constructed using the real nested folder layout:
    #        {base_dir}/results_{strain}_{rq2_dataset_id}/flux_analysis/
    #            reaction_flux_comparison_extended.csv
    #   3. Legacy flat layout (backward compat for standalone use):
    #        {base_dir}/RQ2_{strain}_reaction_flux_comparison_extended.csv
    # ------------------------------------------------------------------
    explicit_strain_files: dict = {}
    if args.strain_files:
        for entry in args.strain_files.split(','):
            entry = entry.strip()
            if ':' in entry:
                # Handle Windows-style absolute paths (C:\...) — split on first ':'
                # then rejoin anything after that in case path contains ':'.
                parts = entry.split(':', 1)          # ['Strain', '/path/...']
                # BUT 'C:\path' would look like ['C', '\path']; detect by checking
                # if the key part is a single letter (drive letter, not a strain name).
                if len(parts[0]) == 1 and parts[0].isalpha():
                    # Looks like a Windows drive letter — try splitting on the SECOND ':'
                    second_colon = entry.find(':', 2)
                    if second_colon != -1:
                        strain_key = entry[:second_colon].rsplit(',', 1)[-1].strip()
                        file_path  = entry[second_colon + 1:]
                        explicit_strain_files[strain_key] = file_path
                    # else: can't parse — skip silently
                else:
                    explicit_strain_files[parts[0]] = parts[1]
            # entries without ':' are malformed — skip silently
        if explicit_strain_files:
            logger.info(f'  Using {len(explicit_strain_files)} explicit strain file paths')

    dataset_id = getattr(args, 'rq2_dataset_id', 'GSE182668')
    ...
    for strain in strains:
        # --- Tier 1: explicit path from orchestrator ---
        if strain in explicit_strain_files:
            rq2_file = explicit_strain_files[strain]
        else:
            # --- Tier 2: new nested layout ---
            rq2_file_nested = os.path.join(
                base_dir,
                f'results_{strain}_{dataset_id}',
                'flux_analysis',
                'reaction_flux_comparison_extended.csv'
            )
            # --- Tier 3: legacy flat layout (backward compat) ---
            rq2_file_legacy = os.path.join(
                base_dir,
                f'RQ2_{strain}_reaction_flux_comparison_extended.csv'
            )
            if os.path.isfile(rq2_file_nested):
                rq2_file = rq2_file_nested
            elif os.path.isfile(rq2_file_legacy):
                rq2_file = rq2_file_legacy
                logger.debug(f'  Using legacy flat path for {strain}')
            else:
                rq2_file = rq2_file_nested   # keep for the error message below
