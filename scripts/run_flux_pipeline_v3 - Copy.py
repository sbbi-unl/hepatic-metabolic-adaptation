...
class FluxPipeline:
    ...
    def run_flux_analysis(self, suffix: str = "") -> Optional[Path]:
        ...
        # CHANGED: Properly nest inside the results directory instead of appending to the string
    ...
    def run_batch_correction(self, input_file: Path, suffix: str = "") -> Optional[Path]:
        ...
        # CHANGED: Explicitly define batch correction plot output dir based on the current branch folder
        ...
        cmd = [
            ...
            '--outdir', str(batch_outdir) # <-- Propagated argument
        ]
    ...
    def run_combined_analysis(self, input_file: Path, suffix: str = "") -> bool:
        ...
        # CHANGED: Explicitly define combined analysis plot output dir based on the current branch folder
        ...
        cmd = [
            ...
            '--input', str(input_file),
            '--output_dir', str(analysis_dir) # <-- Propagated argument
        ]
    ...
    def run_analysis_branch(self, suffix: str = "", run_batch_correction: bool = True) -> bool:
        """Run a complete analysis branch."""
    ...
    def run_pipeline(self) -> bool:
        ...
        if self.run_parallel:
            ...
            self.logger.info("="*80 + "\n")
            success_uncorrected = self.run_analysis_branch('_uncorrected', False)
            
            self.logger.info("\n" + "="*80)
            self.logger.info("BRANCH 2/2: BATCH-CORRECTED ANALYSIS")
            self.logger.info("="*80 + "\n")
            success_corrected = self.run_analysis_branch('_batch_corrected', True)
            ...
            success = self.run_analysis_branch('', True)
    ...
    def save_manifest(self):
        ...
        # CHANGED: Dynamically route the manifests into the designated results directory
...
def main():
    ...
    for key, value in vars(args).items():
        if value is not None and key not in ['config', 'parallel', 'generate_config']:
            config[key] = value
